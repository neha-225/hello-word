...
class Notification_model extends MY_Model {
    ...
    function send_notification($registrationIds, $notificationMsg){   
        ...
        $headers = array(
            'Authorization: key=' . getenv("NOTIFICATION_KEY"), //firebase API key
    ...
    function save_notification($data){
        return $this->common_model->insertData(ALERTS, $data);
    }
     ...
     * Added in ver 2.0
     */
    public function send_push_notification($token_arr, $title, $body, $reference_id,$type,$alert_id,$swap_data='',$looking_id=''){
        if(empty($token_arr)){
            return false;
        }
        //prepare notification payload
        $notif_msg = array('title'=>$title, 'body'=> $body, 'reference_id'=>$reference_id, 'type'=> $type,'swap'=>$swap_data,'looking_id'=>$looking_id,'alertID'=>$alert_id, 'click_action'=>'ChatActivity', 'sound'=>'default');
