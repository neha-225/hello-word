...
class Common_Back_Controller extends MX_Controller {
    ...
    public function __construct(){
        ...
        $this->tbl_users = ADMIN_USERS; //admin table
    }
    ...
    public function check_admin_user_session(){
        ...
        $allowed_pages = array('index','login'); //these pages/methods do not require user  //these pages/methods do not require user authentication
        ...
        if(!is_admin_logged_in() && (in_array($page_slug,$allowed_pages)) && $current_control == $allowed_control){
            ...
            if(!is_admin_logged_in()){
                redirect('admin'); //redirect to admin/login if session not exit
            }
            
            //user session exists
            $user_sess_data = $_SESSION[ $this->admin_user_session_key ]; //user session array

            $session_u_id = $user_sess_data['adminUserID']; //user ID
            $where = array('adminUserID'=>$session_u_id,'status'=>1); //status:1 means active 
